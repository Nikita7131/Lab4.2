...
void memAlloc(int*** Arr, int N, int M); // Ïðîòîòèï Ôóíêö³¿ îðãàí³çàö³¿ âèä³ëåííÿ áëîêó ïàì'ÿò³ äëÿ ìàòðèö³
void printMatrix(int** matr, int countRow, int countColumn); // Ïðîòîòèï Ôóíêö³¿ âèâåäåííÿ ìàòðèö³ íà åêðàí
void transpose(int*** matr, int countRow, int countColumn); // Ïðîòîòèï Ôóíêö³¿ òðàíñïîíóâàííÿ ìàòðèö³
int** MatrixMul(int** matr1, int** matr2, int countRow1, int countRow2, int countColumn1, int countColumn2);// Ïðîòîòèï Ôóíêö³¿ ìíîæåííÿ ìàòðèöü
...
int main() {
  ...
  SetConsoleCP(1251);
  //===================================== ñêàíóºìî ðîçì³ð ìàòðèö³ ===================================== //
  ScanSizeOffMatrix(&Na, &Nb, &Mb);
  //===================================== âèä³ëÿºì ïàì'ÿòü ï³ä ìàòðèö³ ===================================== //
  int** A = NULL;
  memAlloc(&A, Na, Na);

  int** B = NULL;
  memAlloc(&B, Nb, Mb);
  //===================================== ÷èì áóäåì çàïîâíþâàòè ìàòðèö³ ===================================== //
  int variant;
  ...
  int loop = 1;
  do {
    printf("Choose variant:\n 1. Çíàéòè ì³í³ìàëüíå çíà÷åííÿ íèæ÷å ãîëîâíî¿ ä³àãîíàë³ ³ ìàêñèìàëüíå çíà÷åííÿ ìàòðèö³");
    printf("\n 2. Òðàíñïîðòóâàííÿ ìàòðèö³ B");
    printf("\n 3. Ìíîæåííÿ ìàòðèöü A*B");
    printf("\n 4. Ðîçòàøóâàòè âñ³  åëåìåíòè ìàòðèö³ ó ïîðÿäêó çðîñòàííÿ ìàòðèö³ A");
    printf("\n 5. Âèâåäåííÿ íà åêðàí ñóìó åëåìåíò³â ðÿäê³â ìàòðèö³ À òà ñòîâïö³â ìàòðèö³ Á\n");
    ...
    if (variant == 1 ) {
      ...
      for (int i = 0; i < Na; i++) {
        for (int j = 0; j < Na; j++) {
          ...
          if (j < i && Amin > A[i][j]) Amin = A[i][j]; //j < i - íèæ÷å ãîëîâíî¿ ä³àãîíàë³

        }
      }
      printf("A MAX: %d\n", Amax); //ìàêñèìàëüíèé åëåìåíò ìàòðèö³ À
      printf("A MIN under diagonal: %d\n", Amin); //ì³í³ìàëüíèé ç åëåìåíò³â, ùî çíàõîäÿòüñÿ íèæ÷å ãîëîâíî¿ ä³àãîíàë³ ìàòðèö³ À
    }
      ...
      for (int i = 0; i < Na; i++) {
        for (int j = 0; j < Na; j++) {
          for (unsigned long i = 0; i < Na * Na; i++) {
            if (DropList == 0) {
              if (TOP_V[i] < A[i][j]) { // ïåðåâ³ðêà íà ìåíøå çíà÷åííÿ
                DropMark = i; //
                DropList = 1;
              }
            } else { // çäâèã âñüîãî ìàññèâó
              if (TOP_V[NB - i] < A[i][j]) { // áåç öüî¿ ô³ãí³ áóäóòü áàãè
...
void memAlloc(int*** Arr, int N, int M) {
  *Arr = (int**)calloc(N, sizeof(int*)); //!Âèä³ëåííÿ ïàì'ÿò³ ï³ä ïîêàæ÷èêè íà ðÿäêè.  ×åðåç void * calloc (×èñëîÅëåìåíò³â, Ðàçì³ðÅëåìåíòàÂÁàéòàõ);
  if (*Arr == NULL) {
    printf("Error");
    exit(0);
  }
  for (int i = 0; i < N; i++) {
    (*Arr)[i] = (int*)calloc(M, sizeof(int)); //!Âèä³ëåííÿ ïàì'ÿò³ ï³ä çáåð³ãàííÿ ðÿäê³â
    if ((*Arr)[i] == NULL) {
      printf("Error");
      exit(0);
    }
...
void ScanSizeOffMatrix(int *Na, int *Nb, int *Mb) {
  ...
  printf("\nÂâåä³òü ðîçì³ð ìàòðèöü:   \n"); -

  SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 11 | 11);

  printf("\n   Ê³ëüê³ñòü ðÿäê³â-ñòîâáö³â ìàòðèö³ À: ");
  scanf("%d", Na);

  printf("\n   Ê³ëüê³ñòü ðÿäê³â ìàòðèö³ B: ");
  scanf("%d", Nb);

  printf("\n   Ê³ëüê³ñòü ñòîâáö³â ìàòðèö³ B: ");
  scanf("%d", Mb);
...
void ScanContntMatrix(int *variant) {
  ...
  while (Loop) {
    ...
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 15 | 0);
    printf("\n Âèêîðèñòîâóéòå ñòð³ëêè /\ ³ \/ íà êëàâ³àòóð³ äëÿ ïåðåì³ùåííÿ ïî ïóíêòàì ³ Enter äëÿ âèáèðó âàð³àíòà \n\n");

    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 14 | 14);
    printf("\n   Îáåð³òü âàð³àíò íàïîâíåííÿ ìàòðèöü ³íôîðìàö³ºþ:  ");

    X_cursor == 0 ? PrintCLR(1) : PrintCLR(0);
    printf("\n 1. Ââåñòè çíà÷åííÿ âñ³õ êîì³ðîê ñàìîñò³éíî ");

    X_cursor == 1 ? PrintCLR(1) : PrintCLR(0);
    printf("\n 2. Àâòîìàòè÷íî çãåðåíåðóâàòè çà äîïîìîãîþ random() ");

    X_cursor == 2 ? PrintCLR(1) : PrintCLR(0);
    printf("\n 3. Ôîðìóâàííÿ ó â³äïîâ³äíîñò³ äî ìàòåìàòè÷íîãî âèðàçó x + 2");

    X_cursor == 3 ? PrintCLR(1) : PrintCLR(0);
    printf("\n 4. Íà îñíîâ³ çíà÷åííÿ ³íäåêñó åëåìåíòó");
...
void ScanVallOffMatrix(int MbMatrix, int** matr, int countRow, int countColumn) {
  ...
  while (Loop) {
    ...
    switch (getch_vall) { // /\- 72,\/- 80,<- 75,>-77, 13 - enter,1 - 49, 2 - 50; ñòåðòè - 8
      case 72:
        if (Y_Cursor > 0) {
          Y_Cursor --;
        }
